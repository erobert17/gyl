...
class LandingPageController extends Controller
{
    ...
    public function link1($code){
        ...
        if($authorUser->onPlan('monthlyPremium') == true || 
        $authorUser->onPlan('Yearly_Premium') == true || 
        $authorUser->onPlan('Monthly_Unlimited_Users') == true || 
        $authorUser->onPlan('Yearly_Unlimited_Users') == true ||
        $authorUser->onPlan('freeYearTrial') == true || $userRole == 1 || !is_null(Auth::user()) ){// author is subscribed, and current
        ...
        return "This landing page isn't subscribed to yet.";// stop non-logged user.
    }
    ...
    public function openHouseLink($userId, $propertyId){
        ...
        if($userRole != 1 && is_null(Auth::user()) == true){ #if not admin and not subscribed
          if($authorUser->onPlan('monthlyPremium') != true && 
          $authorUser->onPlan('Yearly_Premium') != true &&
          $authorUser->onPlan('Monthly_Unlimited_Users') != true &&
          $authorUser->onPlan('freeYearTrial') != true &&
          $authorUser->onPlan('Yearly_Unlimited_Users') != true){
            return "This landing page hasn't been subscribed to yet by it's creator.";
          }
        }
        ...
        if($authorUser->onPlan('monthlyPremium') == true){// author is subscribed, and current
          ...
          return view('landingPageOpenHouse', compact('user', 'userId', 'thisProperty', 'author', 'authorAvatar', 'properties', 'recentImages', 'propertyId', 'userProfile'));
        }
        ...
        //Must be the author, send a vaiable to be used as a warrning title.
          ...
          return view('landingPageOpenHouse', compact('user','userId', 'thisProperty', 'warningTitle', 'author', 'authorAvatar', 'properties', 'recentImages', 'propertyId', 'userProfile'));
    }
    ...
    public function link4($code){
        ...
        $authorUser->onPlan('Monthly_Unlimited_Users') == true || 
        $authorUser->onPlan('Yearly_Unlimited_Users') == true ||
        $authorUser->onPlan('freeYearTrial') == true || $userRole == 1 || !is_null(Auth::user()) ){
    ...
    public function link6($code){
        ...
        return "This landing page hasn't been subscribed to yet by it's creator.";//top non-logged user.

    }

        /* Landing Page 7 digital download */
    ...
    /* Landing Page 8, appointments */
    public function link8($code){
        $user = DB::table('users')->where('randomUserLink', '=', $code)->take(1)->get();
        $user = $user[0];
        ...
        $thisLandingPageRow = DB::table('landingPages')->where('user_id', '=', $userId)->where('type', '=','Appointment')->get();
        
        // record page visit !!!!!!
        DB::table('views')->insert(['userId' => $userId, 'type' => 'Appointment']);
        ...
        $authorUser->onPlan('freeYearTrial') == true || $userRole == 1 || !is_null(Auth::user()) ){
          ...
          return view('landingPageAppointments', compact('userId', 'author', 'background','productImg','userProfile', 'thisLandingPageRow', 'user'));
        }

        return "This landing page hasn't been subscribed to yet by it's creator.";
    }

    /* Landing Page 9, mailinglist */
    public function link9($code){
        $user = DB::table('users')->where('randomUserLink', '=', $code)->take(1)->get();
        $user = $user[0];
        ...
        $thisLandingPageRow = DB::table('landingPages')->where('user_id', '=', $userId)->where('type', '=','Mailing List')->get();

        $landingPageNumber = '9';
        // record page visit !!!!!!
        DB::table('views')->insert(['userId' => $userId, 'type' => 'Mailing List']);
        ...
        $authorUser->onPlan('freeYearTrial') == true || $userRole == 1 || !is_null(Auth::user()) ){
          ...
          return view('landingPageMailingList', compact('userId', 'author', 'background','productImg','userProfile', 'thisLandingPageRow', 'landingPageNumber', 'user'));
        }
    ...
    public function editLandingPage7(){
        ...
        if(Auth::user()->subscribed('main') == false){
          ...
          $warningTitle = "You haven't <a href='/subscribe'>subscribed</a> yet. This landing page's link will only work for others once you subscribe.";
          //var_dump($warningTitle);
          return view('pages.landingPageDigitalDownloadEdit', compact('landingPage', 'userLink', 'user', 'warningTitle', 'landingPageNumber'));
        }
        return view('pages.landingPageDigitalDownloadEdit', compact('landingPage', 'userLink', 'user', 'landingPageNumber'));
    }

    //Appointment
    public function editLandingPage8(){
        $user = Auth::user();
        $userId = $user->id;
        $landingPage = DB::table('landingPages')->where(['user_id' => $user->id, 'type' => 'Appointment'])->get();
        $userLink = DB::table('users')->where('id', $user->id)->take(1)->get();
        if(isset($userLink[0])){
          $userLink = $userLink[0]->randomUserLink;
        }else{
        ...
        if(Auth::user()->subscribed('main') == false){
          ...
          $warningTitle = "You haven't <a href='/subscribe'>subscribed</a> yet. This landing page's link will only work for others once you subscribe.";
          //var_dump($warningTitle);
          return view('pages.landingPageAppointmentsEdit', compact('landingPage', 'userLink', 'user', 'warningTitle', 'landingPageNumber'));
        }
        return view('pages.landingPageAppointmentsEdit', compact('landingPage', 'userLink', 'user', 'landingPageNumber'));
    }

        //mailing list
    public function editLandingPage9(){
        $user = Auth::user();
        $userId = $user->id;
        $landingPage = DB::table('landingPages')->where(['user_id' => $user->id, 'type' => 'Mailing List'])->get();
        $userLink = DB::table('users')->where('id', $user->id)->take(1)->get();
        if(isset($userLink[0])){
          $userLink = $userLink[0]->randomUserLink;
        }else{
        ...
        if(Auth::user()->subscribed('main') == false){
          ...
          $warningTitle = "You haven't <a href='/subscribe'>subscribed</a> yet. This landing page's link will only work for others once you subscribe.";
          //var_dump($warningTitle);
          return view('pages.landingPageMailingListEdit', compact('landingPage', 'userLink', 'user', 'warningTitle', 'landingPageNumber'));
        }
        return view('pages.landingPageMailingListEdit', compact('landingPage', 'userLink', 'user', 'landingPageNumber'));
    }
    ...
    public function createProperty(Request $request){
      ...
      $details_url = "https://maps.googleapis.com/maps/api/geocode/json?address=" . $cityclean . "&key=AIzaSyBWXBwkxnlRt-vCeT2_rmNjrY07dNnuXEk&sensor=false";
      #return $details_url;
       $curlSession = curl_init();
      curl_setopt($curlSession, CURLOPT_URL, $details_url);
      curl_setopt($curlSession, CURLOPT_BINARYTRANSFER, true);
      curl_setopt($curlSession, CURLOPT_RETURNTRANSFER, true);
      $geoloc = json_decode(curl_exec($curlSession), true);
    ...
    public function saveLandingPage1(Request $request){
        ...
        $secondTitle = str_replace('\"', '"', $request->secondaryTitle);
        ...
        if( count($landingPage) == 0){
            ...
            DB::table('landingPages')->where(['user_id' => $user->id, 'type' => 'Home Valuation'])->update(['title' => $title,
                'secondaryTitle' => $secondTitle]);
        }
    ...
    public function saveLandingPage2(Request $request){
        $user = Auth::user();
        $newTitle = str_replace('\"', '"', $request->title);
        $newSTitle = str_replace('\"', '"', $request->secondaryTitle);
    ...
    public function saveLandingPage7(Request $request){// coupon
        ...
        $disclaimer = $request->disclaimer;
        $titleShadow = $request->titleTextShadow;
        $digitalDownloadUrl = $request->digitalDownloadUrl;
        ...
        DB::table('landingPages')->where(['user_id' => $user->id, 'type' => 'Digital Download'])->update(['title' => $newTitle,
          ...
          'disclaimer' => $disclaimer,
          'titleShadow' => $titleShadow,
          'digitalDownloadUrl' => $digitalDownloadUrl]);
    ...
    public function saveLandingPage8(Request $request){// appointments
        $user = Auth::user();
        $newTitle = str_replace('\"', '"', $request->title);
        $newSTitle = str_replace('\"', '"', $request->secondaryTitle);
        $disclaimer = $request->disclaimer;
        ...
        DB::table('landingPages')->where(['user_id' => $user->id, 'type' => 'Appointment'])->update(['title' => $newTitle,
          'secondaryTitle' => $newSTitle,
          'disclaimer' => $disclaimer,
          'titleShadow' => $titleShadow]);

        if( strlen($request->titleColor) > 2 || strlen($request->disclaimerColor) > 2 || strlen($request->backgroundColor) > 2 || strlen($request->buttonColor) > 2 ){
           DB::table('landingPages')->where(['user_id' => $user->id, 'type' => 'Appointment'])->update([
           'titleColor' => $request->titleColor,
           'disclaimerColor' => $request->disclaimerColor,
           'backgroundColor' => $request->backgroundColor,
           'buttonColor' => $request->buttonColor]);     
        ...
        if( count($landingPage) == 0){
            DB::table('landingPages')->where(['user_id' => $userID, 'type' => 'Appointment'])->insert(['user_id' => $user->id ,
                'title' => $request->title,
                'secondaryTitle' => $request->secondaryTitle]);
        }else{
            DB::table('landingPages')->where(['user_id' => $userID, 'type' => 'Appointment'])->update(['title' => $request->title,
                'secondaryTitle' => $request->secondaryTitle]);
        }
        return redirect()->action('LandingPageController@editLandingPage8');
    }

    public function saveLandingPage9(Request $request){// appointments
        $user = Auth::user();
        $newTitle = str_replace('\"', '"', $request->title);
        $newSTitle = str_replace('\"', '"', $request->secondaryTitle);
        $disclaimer = $request->disclaimer;
        ...
        DB::table('landingPages')->where(['user_id' => $user->id, 'type' => 'Mailing List'])->update(['title' => $newTitle,
          'secondaryTitle' => $newSTitle,
          'disclaimer' => $disclaimer,
          'titleShadow' => $titleShadow]);

        if( strlen($request->titleColor) > 2 || strlen($request->disclaimerColor) > 2 || strlen($request->backgroundColor) > 2 || strlen($request->buttonColor) > 2 ){
           DB::table('landingPages')->where(['user_id' => $user->id, 'type' => 'Mailing List'])->update([
           'titleColor' => $request->titleColor,
           'disclaimerColor' => $request->disclaimerColor,
           'backgroundColor' => $request->backgroundColor,
           'buttonColor' => $request->buttonColor]);     
        ...
        if( count($landingPage) == 0){
            DB::table('landingPages')->where(['user_id' => $userID, 'type' => 'Mailing List'])->insert(['user_id' => $user->id ,
                'title' => $request->title,
                'secondaryTitle' => $request->secondaryTitle]);
        }else{
            DB::table('landingPages')->where(['user_id' => $userID, 'type' => 'Mailing List'])->update(['title' => $request->title,
                'secondaryTitle' => $request->secondaryTitle]);
        }
        return redirect()->action('LandingPageController@editLandingPage9');
    }
    ...
    public function submitEmailDigitalDownload(Request $request){
      $userId=$request->userId;
      $email=$request->email;
      DB::table('leads')->insert(['user_id' => $userId, 'type' => 'Digital Download', 'email' => $email]);
    ...
    public function submitAppointment(Request $request){
      ...
      DB::table('leads')->insert(['user_id' => $userId, 'type' => 'Appointment', 'name' => $name,'email' => $email, 'cell' => $phone, 'timeDate' => $timeDate]);
      
      return "Thank you for your appointment submission. We'll confirm this date & time with you via your E-mail. You can close this window. ";
    }
    ...
    public function landingPage3Stats(){// landing page 3
      ...
      echo 'storedLastLeads '. $storedLastLeads ;
      if( count($storedLeads) > 0){
        $storedLeads = $storedLeads[0];
    ...
    public function landingPage7Stats(){//digital download

      $user = Auth::user();
      $leads = DB::table('leads')->where(['user_id' => $user->id, 'type' => 'Digital Download'])->orderBy('date', 'desc')->get();
      $views = DB::table('views')->where(['userId' => $user->id, 'type' => 'Digital Download'])->get();
      ...
      $difference7 = 0;
      $storedLeads = DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Digital Download'])->pluck('leads')->toArray();
      $storedLastLeads = DB::table('leads')->where(['user_id' => $user->id, 'type' => 'Digital Download'])->orderBy('date', 'desc')->count();
      if( count($storedLeads) > 0){
        $storedLeads = $storedLeads[0];
      }else{// if row doesn't exist in leadsLastLogin, add this lp type to row
        DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Digital Download'])->insert(['userId' =>$user->id, 'leads' => $storedLastLeads, 'difference' => $difference7, 'type' => 'Digital Download']);
      }
      if($storedLeads < $storedLastLeads){//Leads have changed
        $difference7 = ($storedLeads - $storedLastLeads) * -1;
        DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Digital Download'])->update(['leads' => $storedLastLeads, 'difference' => $difference7]);
      }
      
      return view('landingPageStats7DigitalDownload', compact('user', 'leads', 'views'));
    }

    public function landingPage8Stats(){//Appointment

      $user = Auth::user();
      $leads = DB::table('leads')->where(['user_id' => $user->id, 'type' => 'Appointment'])->get();
      $views = DB::table('views')->where(['userId' => $user->id, 'type' => 'Appointment'])->get();

      $difference8 = 0;
      $storedLeads = DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Appointment'])->pluck('leads')->toArray();
      $storedLastLeads = DB::table('leads')->where(['user_id' => $user->id, 'type' => 'Appointment'])->orderBy('date', 'desc')->count();
      if( count($storedLeads) > 0){
        $storedLeads = $storedLeads[0];
      }else{// if row doesn't exist in leadsLastLogin, add this lp type to row
        DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Appointment'])->insert(['userId' =>$user->id, 'leads' => $storedLastLeads, 'difference' => $difference8, 'type' => 'Appointment']);
      }
      if($storedLeads < $storedLastLeads){//Leads have changed
        $difference8 = ($storedLeads - $storedLastLeads) * -1;
        DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Appointment'])->update(['leads' => $storedLastLeads, 'difference' => $difference8]);
      }
      
      return view('landingPageStats8Appointments', compact('leads','views'));
    }

    public function landingPage9Stats(){//Join Email List

      $user = Auth::user();
      $leads = DB::table('leads')->where(['user_id' => $user->id, 'type' => 'Mailing List'])->orderBy('date', 'desc')->get();
      $views = DB::table('views')->where(['userId' => $user->id, 'type' => 'Mailing List'])->get();
      ...
      $difference9 = 0;
      $storedLeads = DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Mailing List'])->pluck('leads')->toArray();
      $storedLastLeads = DB::table('leads')->where(['user_id' => $user->id, 'type' => 'Mailing List'])->orderBy('date', 'desc')->count();
      if( count($storedLeads) > 0){
        $storedLeads = $storedLeads[0];
      }else{// if row doesn't exist in leadsLastLogin, add this lp type to row
        DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Mailing List'])->insert(['userId' =>$user->id, 'leads' => $storedLastLeads, 'difference' => $difference7, 'type' => 'Mailing List']);
      }
      if($storedLeads < $storedLastLeads){//Leads have changed
        $difference9 = ($storedLeads - $storedLastLeads) * -1;
        DB::table('leadsLastLogin')->where(['userId' => $user->id, 'type' => 'Mailing List'])->update(['leads' => $storedLastLeads, 'difference' => $difference9]);
      }
      
      return view('landingPageStats9MailingList', compact('user', 'leads', 'views'));
    }

    public function submitEmailLp9(Request $request){
      $userId=$request->userId;
      $email=$request->email;
      DB::table('leads')->insert(['user_id' => $userId, 'type' => 'Mailing List', 'email' => $email]);
    ...
    public function uploadProductImg7() {
        if(Input::hasFile('file')) {
          ...
          $background       = Input::file('file');
          //$filename     = 'backgroundLP2.' . $background->getClientOriginalExtension();
          $filename     = 'uploadDigitalDownloadProductImg.png';
    ...
    /* lp8 product img upload*/
    public function uploadProductImg8() {
        if(Input::hasFile('file')) {

          $currentUser  = \Auth::user();
          $background       = Input::file('file');
          //$filename     = 'backgroundLP2.' . $background->getClientOriginalExtension();
          $filename     = 'uploadAppointmentProductImg.png';
    ...
    public function uploadDigitalDownload() {
        if(Input::hasFile('file')) {

          $currentUser  = \Auth::user();
          $background       = Input::file('file');
          //$filename     = 'backgroundLP2.' . $background->getClientOriginalExtension();
          $filename     = 'uploadDigitalDownload.png';
          
          $save_path = public_path('/uploads/users/id/'.$currentUser->id.'/digitalDownload/');
          $localSavePath = '/uploads/users/id/'.$currentUser->id.'/digitalDownload/'.$filename;
          $localSavePath = (string)$localSavePath;
          //$save_path = '../uploads/users/id/'.$currentUser->id.'/';
          $path         = $save_path . $filename;
          ...
          Image::make($background->getRealPath())->save($path);
            ...
            DB::table('landingPages')->where(['user_id' => $currentUser->id, 'type' => 'Digital Download'])->update(['digitalDownloadName' => $localSavePath]);
      ...
      public function uploadBackground8() {
        if(Input::hasFile('file')) {

          $currentUser  = \Auth::user();
          $background       = Input::file('file');
          //$filename     = 'backgroundLP2.' . $background->getClientOriginalExtension();
          $filename     = 'backgroundLP8.jpg';
          $save_path    = storage_path() . '/users/id/' . $currentUser->id . '/uploads/images/currentUser/';
          $save_path = public_path('/uploads/users/id/'.$currentUser->id.'/');
          //$save_path = '../uploads/users/id/'.$currentUser->id.'/';
          $path         = $save_path . $filename;
          ...
          //Image::make($background->getRealPath())->save($save_path . 'background.jpg');
            Image::make($background->getRealPath())->save($save_path . $filename);
            DB::table('landingPages')->where(['user_id' => $currentUser->id, 'type' => 'Appointment'])->update(['background' => $save_path.$filename]);
    ...
    public function uploadProductImg9() {
        if(Input::hasFile('file')) {

          $currentUser  = \Auth::user();
          $background       = Input::file('file');
          //$filename     = 'backgroundLP2.' . $background->getClientOriginalExtension();
          $filename     = 'uploadMailingListProductImg.png';
          ...
          Image::make($background->getRealPath())->save($path);
            ...
            DB::table('landingPages')->where(['user_id' => $currentUser->id, 'type' => 'Mailing List'])->update(['productImg' => $localSavePath]);
    ...
    public function uploadBackground9() {
        if(Input::hasFile('file')) {

          $currentUser  = \Auth::user();
          $background       = Input::file('file');
          //$filename     = 'backgroundLP2.' . $background->getClientOriginalExtension();
          $filename     = 'backgroundLP9.jpg';
          #$save_path    = storage_path() . '/users/id/' . $currentUser->id . '/uploads/images/currentUser/';
          $save_path = public_path('/uploads/users/id/'.$currentUser->id.'/');
          //$save_path = '../uploads/users/id/'.$currentUser->id.'/';
          $path         = $save_path . $filename;
          ...
          //Image::make($background->getRealPath())->save($save_path . 'background.jpg');
            Image::make($background->getRealPath())->save($save_path . $filename);
            DB::table('landingPages')->where(['user_id' => $currentUser->id, 'type' => 'Mailing List'])->update(['background' => $save_path.$filename]);
